...
import 'package:editable_pin_field/widget/pin_code_editable_text.dart';
...
typedef InputCounterWidgetBuilder = Widget Function(
  ...
  /// Whether or not the PinCodeTextField is currently focused.  Mainly provided for
...
/// one of its ancestors to be a [Material] widget. When the [PinCodeTextField] is
/// tapped an ink splash that paints on the material is triggered, see
/// [ThemeData.splashFactory].
///
/// To integrate the [PinCodeTextField] into a [Form] with other [FormField] widgets,
...
///
/// {@tool sample}
/// This example shows how to create a [PinCodeTextField] that will obscure input. The
...
///    [PinCodeTextField]. The [EditableText] widget is rarely used directly unless
...
class PinCodeTextField extends StatefulWidget {
  ...
  /// [PinCodeTextField.noMaxLength] then only the current length is displayed.
  ///
  ...
  const PinCodeTextField({
    ...
    @required this.onBackspacePressedOnEmptyField,
       ...
       assert(maxLength == null || maxLength == PinCodeTextField.noMaxLength || maxLength > 0),
  ...
  /// showing when it is tapped by calling [PinCodeEditableTextState.requestKeyboard()].
  ...
  /// to [PinCodeTextField.noMaxLength] then only the current character count is displayed.
  ///
  /// After [maxLength] characters have been input, additional input
  /// is ignored, unless [maxLengthEnforced] is set to false. The text field
  /// enforces the length with a [LengthLimitingTextInputFormatter], which is
  ...
  ///
  /// This value must be either null, [PinCodeTextField.noMaxLength], or greater than 0.
  /// If null (the default) then there is no limit to the number of characters
  /// that can be entered. If set to [PinCodeTextField.noMaxLength], then no limit will
  ...
  /// Callback that can be used to request focus change 
  /// if user presses backspace on PinCodeField
...
class _PinCodeTextFieldState extends State<PinCodeTextField> with AutomaticKeepAliveClientMixin {
  final GlobalKey<PinCodeEditableTextState> _editableTextKey = GlobalKey<PinCodeEditableTextState>();
  ...
  PinCodeEditableTextState get _editableText => _editableTextKey.currentState;
  ...
  Widget build(BuildContext context) {
    ...
    Widget child = RepaintBoundary(
      child: PinCodeEditableText(
        ...
        onBackspacePressedOnEmptyField: widget.onBackspacePressedOnEmptyField,
      ),
    );
