...
/// PinCodeEditableText receives focus. For example, providing iOS's "emergencyCall"
...
class PinCodeEditableText extends StatefulWidget {
  ...
  ///
  /// The cursor refers to the blinking caret when the [PinCodeEditableText] is focused.
  ///
  /// See also:
  ///
  ...
  ///
  /// The [PinCodeEditableText] widget used on its own will not trigger the display
  /// of the selection toolbar by itself. The toolbar is shown by calling
  /// [PinCodeEditableTextState.showToolbar] in response to an appropriate user event.
  ///
  /// See also:
  ///
  ///  * [CupertinoTextField], which wraps an [PinCodeEditableText] and which shows the
  ///    selection toolbar upon user events that are appropriate on the iOS
  ///    platform.
  ///  * [TextField], a Material Design themed wrapper of [PinCodeEditableText], which
  ...
  /// testing purposes.
  ///
  /// It does not affect the necessity to focus the PinCodeEditableText for the cursor
  ...
  @override
  PinCodeEditableTextState createState() => PinCodeEditableTextState();
...
/// State for a [PinCodeEditableText].
class PinCodeEditableTextState extends State<PinCodeEditableText>
    with
        AutomaticKeepAliveClientMixin<PinCodeEditableText>,
        WidgetsBindingObserver,
        TickerProviderStateMixin<PinCodeEditableText>
  ...
  @override
  void didUpdateWidget(PinCodeEditableText oldWidget) {
  ...
  void updateEditingValue(TextEditingValue value) {
    ...
    if (value.text.length == 2) {
      ...
      //the blank space could get replaced by number value. for such case make sure first digit is space.
    ...
    if (value.text.length == 0) {
      //user tried to remove the leading space
      value = value.copyWith(
          text: " ",
          selection:
              TextSelection.fromPosition(TextPosition(offset: " ".length)));

      if (widget.onBackspacePressedOnEmptyField != null) {
        widget.onBackspacePressedOnEmptyField();
      }
    }
  ...
  void performAction(TextInputAction action) {
    switch (action) {
      case TextInputAction.newline:
        // If this is a multiline PinCodeEditableText, do nothing for a "newline"
  ...
  void _startCursorTimer() {
    ...
    if (PinCodeEditableText.debugDeterministicCursor) return;
    if (widget.cursorOpacityAnimates) {
      _cursorTimer =
